...
#performing inner join on the tables
select Persons.Fname, Persons.Lname, Persons.Population, Persons.Rating, Country.Country_name, Country.Population as Country_Population, Country.Area
...
#performing right join on the tables
select Persons.Fname, Persons.Lname, Persons.Population, Persons.Rating, Country.Country_name, Country.Population as Country_Population, Country.Area
from Persons right join Country
on Persons.Country_Id = Country.Id;

#Listing all distinct country names from both the Country and Persons table
select distinct Country_name 
from (select Country_name from Country union select Country_name from Persons)
as Countries;

#listing all countries name from both the country and persons table including duplicates
select Country_name from Country union all select Country_name from Persons;

#Round the rating of all the persons to the nearest integer in the persons table
select concat(Fname, " ", Lname) as Name, Country_name as Country, Population, 
round(Rating) as RoundedRating from Persons;